...
app.post("/sendBook", upload.fields([{ name: "image", maxCount: 1 }, { name: "pdf", maxCount: 1 }]), (req, res) => {
    ...
    const { title, des, cos, pdfTitle } = req.body;
    const image = req.files["image"] ? req.files["image"][0].buffer : null;
    const pdf = req.files["pdf"] ? req.files["pdf"][0].buffer : null;
    console.log(req.files["image"])
    console.log(image)

    let insertBook = "INSERT INTO books (title, description, cost,image ,pdf_title, pdf) VALUES (?,?,?,?,?,?)"

bookDB.query(insertBook, [title,des, cos,image,pdfTitle, pdf], (err, result) => {
...
app.post("/deleteBook",(req,res)=>{
    ...
    console.log(`deleted ${titleName}`)
    bookDB.query(sql,[titleName],(err,result)=>{
...
app.get('/books', (req, res) => {
    const sql = "SELECT id, title, description, cost, pdf_title, image, pdf FROM books";
    bookDB.query(sql, (err, results) => {
        ...
        const booksWithBase64 = results.map((book) => ({
            id: book.id,
            title: book.title,
            description: book.description,
            cost: book.cost,
            pdf_title: book.pdf_title,
            image: book.image ? `data:image/jpeg;base64,${book.image.toString("base64")}` : null,
            pdf: book.pdf ? `data:application/pdf;base64,${book.pdf.toString("base64")}` : null
        }));
